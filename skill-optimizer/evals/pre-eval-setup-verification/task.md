...
The developer experience team at a software consultancy has been asked to create a repeatable onboarding kit for engineers who are new to running Tessl plugin evaluations. Several engineers have recently hit walls trying to kick off evaluation runs — some were pointing at the wrong directory, others didn't realize they needed active credentials, and a few had plugins with no scenarios at all. Each of these blockers is only discovered mid-run, wasting time.
...
- `preflight.sh` — An executable shell script that checks all prerequisites. It should print clear status messages for each check and stop (exit non-zero) with a helpful message if a required condition is not met. It should accept the plugin path as an argument (e.g., `./preflight.sh ./my-plugin`).

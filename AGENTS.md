...
| `review-lenses/<lens>/SKILL.md` | A repository-specific lens available to Code Review. Not a plugin, and not published. |
...
- There is no in-repository Code Review workflow. `.tessl-code-review.yml` and `review-lenses/` configure reviews when a reviewer invokes them; do not assume they provide an automatic pull-request check or merge gate.

The lenses in `review-lenses/` and the profile that routes them are reviewer instructions. Review a change to either with the same care as a change to a published skill, especially changes that narrow profile routing or weaken a lens.
